...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dh2, dW2, db2 = affine_backward(dout, cache_h2)
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(self.num_layers):
        ...
        if i == self.num_layers-1: 
            h, results['affine'+str(i+1)] = affine_forward(h, W, b)
        else:
            h, results['affine'+str(i+1)] = affine_forward(h, W, b)
            if self.use_batchnorm:
                gamma, beta = self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)]
                h, results['batch'+str(i+1)] = batchnorm_forward(h, gamma, beta, self.bn_params[i])
            h, results['relu'+str(i+1)] = relu_forward(h)
            if self.use_dropout:
                h, results['drop'+str(i+1)] = dropout_forward(h, self.dropout_param)
    ...
    ############################################################################
    pass
    loss, dout = softmax_loss(h, y)
    loss += reg

    dh, dW, db = 0, 0, 0
    for i in xrange(self.num_layers-1, -1, -1):
        if i == self.num_layers-1:
            dh, dW, db = affine_backward(dout, results['affine'+str(i+1)])
        else:
            if self.use_dropout:
                dh = dropout_backward(dh, results['drop'+str(i+1)])
            dh = relu_backward(dh, results['relu'+str(i+1)])
            if self.use_batchnorm:
                dh, dgamma, dbeta = batchnorm_backward(dh, results['batch'+str(i+1)])
                grads['gamma'+str(i+1)] = dgamma
                grads['beta'+str(i+1)] = dbeta
            dh, dW, db = affine_backward(dh, results['affine'+str(i+1)])
